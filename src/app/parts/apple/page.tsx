...
import { Grid, List, SlidersHorizontal, ArrowRight, ChevronLeft, ChevronRight, Check } from 'lucide-react';

interface ApplePartsPageProps {
  searchParams: { [key: string]: string | string[] | undefined };
}

// Get Apple products with filters applied
async function getFilteredAppleProducts(filters: FilterState, page: number = 1, limit: number = 24) {
  ...
  if (error) {
    console.error('Error fetching filtered Apple products:', error);
  ...
  const transformedProducts: Product[] = (products || []).map((product: any) => ({
    ...
    category: product.category_id || 'apple-parts',
...
export default async function ApplePartsPage({ searchParams }: ApplePartsPageProps) {
  ...
  const { products, totalCount } = await getFilteredAppleProducts(currentFilters, currentPage);

  // Calculate filter options (we need all products to calculate counts, not just paginated ones)
  let allProducts: Product[] = [];
  try {
    const result = await getFilteredAppleProducts(currentFilters, 1, 1000); // Get more for accurate counts
    allProducts = result.products;
  } catch (error) {
    console.error('Error getting all products for filter counts:', error);
    allProducts = products; // fallback to current page products
  }
        ...
        <div className="max-w-7xl mx-auto px-4 py-16">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">
              Apple iPhone, iPad, Watch & MacBook Parts
            </h1>
            <p className="text-xl md:text-2xl mb-8 text-gray-300 max-w-3xl mx-auto">
              Genuine OEM • Premium Quality • Fast USA Shipping • Expert Support
            </p>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6">
              <p className="text-gray-600">
                {totalCount} Apple part{totalCount !== 1 ? 's' : ''} found
              </p>
            </div>
